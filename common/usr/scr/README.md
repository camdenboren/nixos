...
| ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [audioPreventsLock.nix](audioPreventsLock.nix) | Loops and inhibits GNOME session idle while any PulseAudio sink is RUNNING, using `gnome-session-inhibit`. Embeds the path of `pulseaudio`'s `pactl`.                                                                                                          |
...
| [upsMetrics.nix](upsMetrics.nix)               | Queries NUT via `upsc` for input/battery voltage and charge; emits one JSON line. Uses smart1500lcd UPS on `media`, bn1500m2 elsewhere.                                                                                                                        |
