...
let
  defaultUser = "outline";
  cfg = config.services.custom-outline;
...
in
{
  ...
  options.services.custom-outline = {
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.redis.servers.outline = lib.mkIf (cfg.redisUrl == "local") {
      enable = true;
      user = config.services.custom-outline.user;
    ...
    systemd.services.outline =
      ...
      in
      {
        ...
        script = ''
          ...
          # from https://github.com/NixOS/nixpkgs/pull/536063/files
          ${
