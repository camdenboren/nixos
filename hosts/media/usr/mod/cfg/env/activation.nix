  ...
  home.activation = {
    replaceConfigs = lib.hm.dag.entryAfter [ "installPackages" ] ''
      if test -d ~/.config/FreeTube; then
        ...
        $DRY_RUN_CMD rm -f $VERBOSE_ARG ~/.config/FreeTube/settings.db
        ...
        $DRY_RUN_CMD cp -r $VERBOSE_ARG ~/etc/nixos/hosts/media/usr/dot/FreeTube/settings.db ~/.config/FreeTube
